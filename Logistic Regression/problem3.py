...
def compute_a(z):
    ...
    c = z.shape[0]
    max_z = max(z)[0,0]
    a = np.mat(np.zeros((c,1)))
    temp_z = np.mat(np.zeros((c,1)))
    for i in range(c):
        temp_z[i,0] = z[i,0] - max_z
    sum_temp_z = sum(math.exp(temp_z[x,0]) for x in range(c))
    for i in range(c):
        a[i,0] = math.exp(temp_z[i,0]) / sum_temp_z
...
def compute_L(a,y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    L = - math.log(a[y,0]) if a[y,0] != 0 else 1e5 + 10
...
def compute_dL_da(a, y):
    ...
    ## INSERT YOUR CODE HERE     
    c = a.shape[0]
    dL_da = np.mat(np.zeros((c,1)))
    dL_da[y,0] = - 1 / a[y,0] if a[y,0] != 0 else -1e5 - 10
...
def compute_da_dz(a):
    ...
    ## INSERT YOUR CODE HERE
    c = a.shape[0]
    da_dz = np.mat(np.zeros((c,c)))
    for i in range(c):
        for j in range(c):
            da_dz[i,j] = a[i,0] * (1 - a[i,0]) if i == j else - a[i,0] * a[j,0]
...
def compute_dz_dW(x,c):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dz_dW = np.mat(np.zeros((c,x.shape[0])))
    for i in range(c):
        dz_dW[i,:] = x.T
...
def compute_dL_dW(dL_dz,dz_dW):
    ...
    '''dL_dW = np.mat(np.zeros(dz_dW.shape))
        for i in range(dL_dW.shape[0]):
            dL_dW[i,:] = dL_dz[i,0] * dz_dW[i,:]'''

    #c, p = dz_dW.shape
    #dL_dW = np.matrix([(dL_dz[i,0] * dz_dW[i]).getA()[0] for i in range(c)])
...
def compute_dL_db(dL_dz,dz_db):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    #dL_db = np.mat([dL_dz[i,0] * dz_db[i,0] for i in range(dL_dz.shape[0])]).T
